...
@interface CRFInvestProductCell()
...
@end
@implementation CRFInvestProductCell
...
- (void)setProduct:(CRFProductModel *)product {
    _product = product;
//    self.productTagLabel.text = product.tipsStart;
//    self.productTagLabel.textColor = [UIColor orangeColor];
//    self.productTagLabel.layer.borderColor = [UIColor yellowColor].CGColor;
    [self setTips];
...
//    [self.leftLabel setAttributedText:[self setTitleMutableString:self.leftLabel.text]];
    ...
    CGFloat width = [self.product.name boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 20) fontNumber:16 * kWidthRatio].width;
...
- (NSMutableAttributedString *)setTitleMutableString:(NSString *)total {
    ...
    [attString addAttribute:NSForegroundColorAttributeName value:UIColorFromRGBValue(0xFB4D3A) range:NSMakeRange(0, [a firstObject].length)];
    ...
    [attString addAttribute:NSForegroundColorAttributeName value:UIColorFromRGBValue(0xFB4D3A) range:NSMakeRange([a firstObject].length + 1, 1)];
...
- (void)setContentMutableString {
    ...
    [attString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:16.0f * kWidthRatio] range:NSMakeRange(0, self.product.name.length)];
    ...
    [attString addAttribute:NSFontAttributeName value:[CRFUtils fontWithSize:12.0 * kWidthRatio] range:NSMakeRange(self.rightLabel.text.length - self.product.content.length, self.product.content.length)];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextMoveToPoint(context, kTitleLabelWidth * kWidthRatio + kSpace, CGRectGetMinY(self.rightLabel.frame)+4);  //起点坐标
    CGContextAddLineToPoint(context, kTitleLabelWidth * kWidthRatio + kSpace, CGRectGetMaxY(self.rightLabel.frame)-2);   //终点坐标
