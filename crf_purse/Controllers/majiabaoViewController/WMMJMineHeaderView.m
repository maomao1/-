...
- (void)configView {
    ...
    [bgImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(230 * kHeightRatio);
...
- (void)layoutViews {
    [self.avatarImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self).with.offset(34 * kWidthRatio);
    ...
    [self.totalAmountlabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self).with.offset(78 * kWidthRatio);
    }];
...
- (void)setUserStatus:(UserStatus)userStatus {
    ...
    if (_userStatus == On_line) {
        [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:kUserInfo.headUrl] placeholderImage:[UIImage imageNamed:@"默认头像"]];
        DLog(@"on line");
        NSString *subString = [NSString stringWithFormat:@"%@(%@)",[kUserInfo formatUserName],[kUserInfo formatMobilePhone]];
        NSString *s = nil;
        if (kUserInfo.riskLevel && [kUserInfo.riskLevel integerValue] ==1 ) {
            s = @"保守型";
        } else if (kUserInfo.riskLevel && [kUserInfo.riskLevel integerValue] == 2) {
            s = @"平衡型";
        } else if (kUserInfo.riskLevel && [kUserInfo.riskLevel integerValue] == 3) {
            s = @"进取型";
        } else {
            if ([kUserInfo.accountStatus integerValue] == 1) {
                s = @"存管账户未开通";
            } else{
                s = @"风险承受能力未评估";
            }
...
- (void)changedSecret:(UIButton *)sender {
    ...
    if ([CRFAppManager defaultManager].login && [kUserInfo.accountStatus integerValue] == 2) {
...
- (void)refreshImageView {
    [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:kUserInfo.headUrl] placeholderImage:[UIImage imageNamed:@"默认头像"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        if (error) {
            DLog(@"failed");
        } else {
            DLog(@"success");
