...
- (void)getConfig {
    if ([kUserInfo.accountStatus integerValue] == 2) {
...
- (void)getUserInfo {
    if (self.flag) {
        ...
        self.flag = YES;
        [CRFRefreshUserInfoHandler defaultHandler].userInfo = kUserInfo;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.item) {
            ...
            case 3: {
                NSURL *callUrl = [NSURL
                                  URLWithString:[NSString stringWithFormat:@"tel:400-688-8692"]];
...
- (void)userTransform {
    ...
    if ([CRFAppManager defaultManager].login) {
        if ([kUserInfo.accountStatus integerValue] == 1) {
            ...
            ((CRFRollOutViewController *)controller).bankInfo =  [NSString stringWithFormat:@"%@卡(%@)",[kUserInfo.bankCode getBankCode].bankName,[kUserInfo.openBankCardNo substringFromIndex:kUserInfo.openBankCardNo.length - 4]];
...
- (void)refreshUnreadMessageCount {
    weakSelf(self);
    [[CRFStandardNetworkManager defaultManager] post:[NSString stringWithFormat:APIFormat(kGetUnreadMessagePath),kUserInfo.customerUid] paragrams:nil success:^(CRFNetworkCompleteType errorType, id response) {
        strongSelf(weakSelf);
