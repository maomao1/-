...
@property (nonatomic, strong) CRFLocusView *locusView;
...
-(void)lockViewPrepare{
    ...
    [self setFrame:CGRectMake(0, 0, kScreenWidth - 48 * 2 * kWidthRatio, kScreenWidth - 48 * 2 * kWidthRatio)];
    [self setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2, 368 * kWidthRatio)];
    ...
    self.locusView = [[CRFLocusView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame))];
    [self addSubview:self.locusView];
    NSMutableArray *subViews = [NSMutableArray new];
    for (UIView *sub in self.subviews) {
        if ([sub isKindOfClass:[PCCircle class]]) {
            [subViews addObject:sub];
        }
    }
    self.locusView.locusSubviews = subViews;
    self.locusView.circleView = self;
...
-(void)layoutSubviews{
    ...
    CGFloat itemViewWH = CircleRadius * 2 * kWidthRatio;
    CGFloat marginValue = (self.frame.size.width - 3 * itemViewWH) / 2.0f;
    NSMutableArray *views = [NSMutableArray new];
    for (UIView *v in self.subviews) {
        if ([v isKindOfClass:[PCCircle class]]) {
            [views addObject:v];
        }
    }
    [views enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, BOOL *stop) {
        if ([subview isKindOfClass:[PCCircle class]]) {
