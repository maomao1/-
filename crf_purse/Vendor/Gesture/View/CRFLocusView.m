...
#import "CRFLocusView.h"
#import "PCCircle.h"
#import "PCCircleViewConst.h"

@interface CRFLocusView()
...
{
//    [self.superview exchangeSubviewAtIndex:self.locusSubviews.count - 1 withSubviewAtIndex:0];
    ...
    [self.locusSubviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        ...
        if (self.circleView.type != CircleViewTypeSetting) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self errorToDisplay];
    [self.circleView bringSubviewToFront:self];
}
...
{
    [self.locusSubviews enumerateObjectsUsingBlock:^(PCCircle *obj, NSUInteger idx, BOOL *stop) {
        [obj setAngle:0];
    }];
}
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        // 1.通知代理
        if ([self.circleView.delegate respondsToSelector:@selector(circleView:type:connectCirclesLessThanNeedWithGesture:)]) {
            [self.circleView.delegate circleView:self.circleView type:self.circleView.type connectCirclesLessThanNeedWithGesture:gesture];
        }
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [CRFUserDefaultManager saveTemporaryGesture:gesture];
            
            // 通知代理
            if ([self.circleView.delegate respondsToSelector:@selector(circleView:type:didCompleteSetFirstGesture:)]) {
                [self.circleView.delegate circleView:self.circleView type:self.circleView.type didCompleteSetFirstGesture:gesture];
            }
            
        } else { // 接受第二个密码并与第一个密码匹配，一致后存储起来
            
            BOOL equal = [gesture isEqual:[CRFUserDefaultManager getTemporaryGesture]]; // 匹配两次手势
            
            // 通知代理
            if ([self.circleView.delegate respondsToSelector:@selector(circleView:type:didCompleteSetSecondGesture:result:)]) {
                
                [self.circleView.delegate circleView:self.circleView type:self.circleView.type didCompleteSetSecondGesture:gesture result:equal];
...
- (void)gestureEndByTypeLoginWithGesture:(NSString *)gesture length:(CGFloat)length
{
    ...
    if ([self.circleView.delegate respondsToSelector:@selector(circleView:type:didCompleteLoginGesture:result:)]) {
        [self.circleView.delegate circleView:self.circleView type:self.circleView.type didCompleteLoginGesture:gesture result:equal];
    }
