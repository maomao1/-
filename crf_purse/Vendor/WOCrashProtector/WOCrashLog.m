...
+ (void)wo_noteErrorWithException:(NSException *)exception attachedTODO:(NSString *)todo {
    ...
    NSString *mainCallStackSymbolMsg = [self _getMainCallStackSymbolMessageWithCallStackSymbolStr:callStackSymbolsArr[1]];
    if ([self containsUseness:callStackSymbolsArr]) {
        return;
    }
...
+ (BOOL)containsUseness:(NSArray *)list {
    for (NSString *str in list) {
        if ([str containsString:@"JPUSH"]) {
            return YES;
        }
    }
    return NO;
